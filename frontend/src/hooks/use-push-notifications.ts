...
export function usePushNotifications() {
  ...
  const sendTestNotification = useCallback(
    async ({ receiverId, body }: { receiverId: number; body: string }) => {
      // const savedSubscription = localStorage.getItem(STORAGE_KEY);
      //   const savedSubscription = await api.get(
      //     `user/retrieve_permission_token/${receiverId}`
      //   );
      ...
      try {
        // const subscription = JSON.parse(savedSubscription.data.subscription);
        const response = await api.post("user/message_reminder/", {
          message: body,
